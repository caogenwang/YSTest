...
#include "mupdf/pdf.h"
#include <mupdf/swf-text.h>

#define JNI_FN(A) Java_com_docin_mupdf_ ## A
#define PACKAGENAME "com/docin/mupdf"
...
JNI_FN(MuPDFCore_hkwatermarkSet)(JNIEnv * env, jobject thiz, bool watermark)
{
	globals *glo = get_globals(env, thiz);
	if (glo == NULL)
		return;
	fz_show_water_mark(glo->ctx,glo->doc,watermark);
}

JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_hkdarknessSet)(JNIEnv * env, jobject thiz,int theme,jint background,jint text)
{
	globals *glo = get_globals(env, thiz);
	if (glo == NULL)
		return;
	glo->theme = theme;
	glo->background = background;
	glo->text = text;
}

JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_hkcuttingSet)(JNIEnv * env, jobject thiz, bool cutting,jint offset)
{
	globals *glo = get_globals(env, thiz);
	if (glo == NULL)
		return;
...
JNI_FN(MuPDFCore_gotoPageInternal)(JNIEnv *env, jobject thiz, int page)
{
	...
	fz_set_theme(ctx,glo->theme);
	fz_set_backgroundColor(ctx,glo->background);
	fz_set_textColor(ctx,glo->text);
	...
	fz_try(ctx)
	{
		...
		fz_device* list_dev = fz_new_list_device_with_page(ctx, list,pc->page);
		fz_run_page_contents(ctx, pc->page, list_dev, &fz_identity, 0);
		...
		int pageW, int pageH, int patchX, int patchY, int patchW, int patchH)
{
	...
	fz_try(ctx)
	{
		...
		if (glo->cutting)
		{
			cut = cutRect(pc->page->page_rect,pc->page->content_rect);
			if (glo->offset != 0)
			{
				float scale = (cut.x1 - cut.x0) / (cut.y1 - cut.y0);
				float x0,y0,x1,y1;
				int offset_y = glo->offset / scale;
				cut.x0 -= glo->offset;
				cut.x1 += glo->offset;
				cut.y0 -= offset_y;
				cut.y1 += offset_y;
				fz_intersect_rect(&cut,&pc->page->page_rect);
			}
			
			float x_scale = (float)pageW / (cut.x1 - cut.x0);
			float y_scale = (float)pageH / (cut.y1 - cut.y0);

			patchX = patchX + (int)(cut.x0*x_scale);
			patchY = patchY + (int)(cut.y0*y_scale);
		}
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_hktext)(JNIEnv * env, jobject thiz)
{
	LOGI("Native fun hktext");
	jclass textCharClass;
	jclass textSpanClass;
	jclass textLineClass;
	jmethodID ctor;
	jobjectArray larr = NULL;
	...
	//这个list可以在放大的时候复用
	fz_display_list* list = fz_new_display_list(ctx);
	fz_device* list_dev = fz_new_list_device_with_page(ctx, list,page);
	
	fz_run_page_contents(ctx, page, list_dev, &fz_identity, 0);
    
	fz_try(ctx)
	{
		Page_Rect *text_page = (Page_Rect*)page->text_page;
		int i,j;
		LOGI("Native fun %d",text_page->len);
		larr = (*env)->NewObjectArray(env, text_page->len, textLineClass, NULL);
		if (larr == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
		for (i=0; i<text_page->len; i++) {
			Line_Rect* line = &text_page->lines[i];
			jobjectArray *sarr;
			sarr = (*env)->NewObjectArray(env, line->len, textCharClass, NULL);
			if (sarr == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
			for (j = 0; j < line->len; j++) {
				Text_Rect* word = &line->texts[j];
				// if (word->unicode != 32) {
					jobject cobj = (*env)->NewObject(env, textCharClass, ctor, word->rect.x0, word->rect.y0, word->rect.x1, word->rect.y1, word->unicode);
					if (cobj == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectfailed");

					(*env)->SetObjectArrayElement(env, sarr, j, cobj);
					(*env)->DeleteLocalRef(env, cobj);
				// }
			}
		(*env)->SetObjectArrayElement(env, larr, i, sarr);
		(*env)->DeleteLocalRef(env, sarr);
		}
	}
	fz_always(ctx)
	...
	fz_catch(ctx)
	{
		jclass cls = (*env)->FindClass(env, "java/lang/OutOfMemoryError");
		if (cls != NULL)
			(*env)->ThrowNew(env, cls, "Out of memory in MuPDFCore_hktext");
		(*env)->DeleteLocalRef(env, cls);
		return NULL;
	}
	LOGI("Native fun finish");
	return larr;
}

JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_hktransform)(JNIEnv * env, jobject thiz,jobjectArray rects,jint pageW, jint pageH,jint pagenum)
{
	LOGI("Native fun hktransform");
	int i = 0;
	jclass textCharClass;
	jmethodID ctor;
	jobjectArray sarr = NULL;
	float cutx_scale;
	float cuty_scale;
	...
	if (ctor == NULL) 
		return NULL;
	x0_fid = (*env)->GetFieldID(env, textCharClass, "left", "F");
	if (x0_fid == NULL) 
		return NULL;
	y0_fid = (*env)->GetFieldID(env, textCharClass, "top", "F");
	if (y0_fid == NULL) 
		return NULL;
	x1_fid = (*env)->GetFieldID(env, textCharClass, "right", "F");
	if (x1_fid == NULL) 
		return NULL;
	y1_fid = (*env)->GetFieldID(env, textCharClass, "bottom", "F");
	if (y1_fid == NULL) 
		return NULL;
	...
	fz_device* list_dev = fz_new_list_device_with_page(ctx, list,page);
	fz_run_page_contents(ctx, page, list_dev, &fz_identity, 0);
	fz_rect cut = cutRect(page->page_rect,page->content_rect);
	if(glo->cutting)
	{
		if (glo->offset != 0)
		{
			...
			cut.y0 -= offset_y;
			cut.y1 += offset_y;
			fz_intersect_rect(&cut,&page->page_rect);
		}

		cutx_scale = (float)pageW / (cut.x1 - cut.x0);
		cuty_scale = (float)pageH / (cut.y1 - cut.y0);
	}
	else
	{
		cutx_scale = (float)pageW / (page->page_rect.x1 - page->page_rect.x0);
		cuty_scale = (float)pageH / (page->page_rect.y1 - page->page_rect.y0);
	}
	jint size = (*env)->GetArrayLength(env,rects);
	sarr = (*env)->NewObjectArray(env, size, textCharClass, NULL);
	if (sarr == NULL) return NULL;
	for (int i = 0; i < size; i++) {
		jobject TextChar = (*env)->GetObjectArrayElement(env, rects, i);
		x0 = (*env)->GetFloatField(env, TextChar, x0_fid);
		y0 = (*env)->GetFloatField(env, TextChar, y0_fid);
		x1 = (*env)->GetFloatField(env, TextChar, x1_fid);
		y1 = (*env)->GetFloatField(env, TextChar, y1_fid);
		if (glo->cutting)
		{
			x0 = (x0 - cut.x0) * cutx_scale;
			x1 = (x1 - cut.x0) * cutx_scale;
			y0 = (y0 - cut.y0) * cuty_scale;
			y1 = (y1 - cut.y0) * cuty_scale;
		}
		else
		{
			x0 *= cutx_scale;
		...
		jobject cobj = (*env)->NewObject(env, textCharClass, ctor, x0, y0, x1, y1, 0);
		if (cobj == NULL) return NULL;

		(*env)->SetObjectArrayElement(env, sarr, i, cobj);
		(*env)->DeleteLocalRef(env, cobj);
	}
	{
		jclass accesserClass = (*env)->FindClass(env, PACKAGENAME "/MuPDFCore");;
		jmethodID setTransformRects = (*env)->GetMethodID(env,accesserClass, "setTransformRects", "([Lcom/docin/mupdf/TextChar;)V");
		(*env)->CallVoidMethod(env,thiz, setTransformRects, sarr);
		(*env)->DeleteLocalRef(env, sarr);
	}

	if(page != NULL) fz_drop_page(ctx, page);
	if(list_dev != NULL) fz_drop_device(ctx, list_dev);
	if(list != NULL) fz_drop_display_list(ctx, list);
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_hkselect)(JNIEnv * env, jobject thiz,jfloat x0,jfloat y0,jfloat x1,jfloat y1,jint number,jint pageW, jint pageH)
{
	LOGI("Native fun hkselect");
	jclass textCharClass;
	jmethodID ctor;
	jobjectArray sarr = NULL;
	fz_rect oriRects = {0};
	jobjectArray orisarr = NULL;
	...
	fz_rect hkrect = {x0,y0,x1,y1};
	HKSelect *s = hk_go_select(ctx, page,hkrect);

	fz_try(ctx)
	{
		int rectsLen = s->len;
		int i;
		sarr = (*env)->NewObjectArray(env, rectsLen, textCharClass, NULL);
		if (sarr == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");

		orisarr = (*env)->NewObjectArray(env, rectsLen, textCharClass, NULL);
		if (orisarr == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");

		for(i = 0;i < rectsLen; i ++)
		{
			fz_rect line = fz_empty_rect;
			line = s->rects[i];
			...
			jobject oobj = (*env)->NewObject(env, textCharClass, ctor, oriRects.x0, oriRects.y0, oriRects.x1, oriRects.y1, 0);
			if (oobj == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectfailed");

			(*env)->SetObjectArrayElement(env, orisarr, i, oobj);
			(*env)->DeleteLocalRef(env, oobj);

			if (glo->cutting)
			{
				line.x0 = (line.x0 - cut.x0) * cutx_scale;
				line.x1 = (line.x1 - cut.x0) * cutx_scale;
				line.y0 = (line.y0 - cut.y0) * cuty_scale;
				line.y1 = (line.y1 - cut.y0) * cuty_scale;
			...
			jobject cobj = (*env)->NewObject(env, textCharClass, ctor, line.x0, line.y0, line.x1, line.y1, 0);
			if (cobj == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectfailed");
		...
		{
			jclass accesserClass = (*env)->FindClass(env, PACKAGENAME "/MuPDFCore");;
			jmethodID setTextRects = (*env)->GetMethodID(env,accesserClass, "setTextRects", "([Lcom/docin/mupdf/TextChar;)V");
			(*env)->CallVoidMethod(env,thiz, setTextRects, sarr);
			(*env)->DeleteLocalRef(env, sarr);
		}

		{
			jclass accesserClass = (*env)->FindClass(env, PACKAGENAME "/MuPDFCore");;
			jmethodID setRectsInfile = (*env)->GetMethodID(env,accesserClass, "setRectsInfile", "([Lcom/docin/mupdf/TextChar;)V");
			(*env)->CallVoidMethod(env,thiz, setRectsInfile, orisarr);
			(*env)->DeleteLocalRef(env, orisarr);
		}
		...
		{
			jclass accesserClass = (*env)->FindClass(env, PACKAGENAME "/MuPDFCore");;
			jmethodID setStringBuf = (*env)->GetMethodID(env,accesserClass, "setStringBuf", "([B)V");
			jbyteArray bytearray;
			bytearray = (*env)->NewByteArray(env,bufLen);
			(*env)->SetByteArrayRegion(env,bytearray, 0, bufLen, (const jbyte*)s->string->data);
			(*env)->CallVoidMethod(env,thiz, setStringBuf, bytearray);
			(*env)->DeleteLocalRef(env,bytearray);
		}
		hk_drop_select(ctx,s);
	}
	fz_always(ctx)
	{
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_hksearch)(JNIEnv * env, jobject thiz,jintArray uArray,jint number,jint pageW, jint pageH)
{
	LOGI("Native fun hksearch");
	jclass textCharClass;
	jmethodID ctor;
	jmethodID stor;
	
	jclass searchClass;
	jobjectArray searcharr = NULL;
	...
	fz_buffer* key = fz_new_buffer(ctx, 0);
    fz_output *out = fz_new_output_with_buffer(ctx, key);
	{
		jsize size = (*env)->GetArrayLength(env,uArray);
		jint *data = (*env)->GetIntArrayElements(env,uArray,NULL);
		for (int j = 0; j < size; j++) {
			unsigned short unicode = (unsigned short)data[j];
			if (unicode != 32)
			{
				fz_write(ctx, out, &unicode, 2);
			}
		}
	...
	textCharClass = (*env)->FindClass(env, PACKAGENAME "/TextChar");
	if (textCharClass == NULL) return NULL;
	searchClass = (*env)->FindClass(env, "com/docin/HKSearch/HKSearch");
	if (searchClass == NULL) return NULL;

	ctor = (*env)->GetMethodID(env, textCharClass, "<init>", "(FFFFC)V");
	if (ctor == NULL) return NULL;
	stor = (*env)->GetMethodID(env, searchClass, "<init>", "([Lcom/docin/mupdf/TextChar;[B)V");
	...
	fz_device* list_dev = fz_new_list_device_with_page(ctx, list,page);
	fz_run_page_contents(ctx, page, list_dev, &fz_identity, 0);
	HKSearch *search = hk_go_search(ctx, page,key);
	int selcount = search->len;
	fz_rect cut = cutRect(page->page_rect,page->content_rect);
	if(glo->cutting)
	{
		if (glo->offset != 0)
	...
	fz_try(ctx)
	{
		searcharr = (*env)->NewObjectArray(env, selcount, searchClass, NULL);
		if (searcharr == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
		for (int j = 0; j < selcount; j++)
		{
			HKSelect select = search->select[j];
			int rectsLen = select.len;
			...
			int bufLen = select.string->len;
			// for (int m = 0; m < bufLen;m=m+sizeof(unsigned short) ) {
			// 	unsigned short unicode = (select.string->data[m+1]<<8) + select.string->data[m];
			// 	char* utf8 = getUTF8(unicode);
			// 	LOGI("unicode:%s\n",utf8);
			// }
			...
			jbyteArray bytearray;
			bytearray = (*env)->NewByteArray(env,bufLen);
			(*env)->SetByteArrayRegion(env,bytearray, 0, bufLen, (const jbyte*)select.string->data);

			jobject sobj = (*env)->NewObject(env, searchClass,stor,sarr,bytearray);
			if (sobj == NULL) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectfailed");

			(*env)->DeleteLocalRef(env,bytearray);
			(*env)->DeleteLocalRef(env, sarr);
			
			(*env)->SetObjectArrayElement(env, searcharr, j, sobj);
			(*env)->DeleteLocalRef(env, sobj);
		}

		jclass accesserClass = (*env)->FindClass(env, PACKAGENAME "/MuPDFCore");;
		jmethodID setSearchRects = (*env)->GetMethodID(env,accesserClass, "setSearchRects", "([Lcom/docin/HKSearch/HKSearch;)V");
		(*env)->CallVoidMethod(env,thiz, setSearchRects, searcharr);
		(*env)->DeleteLocalRef(env, searcharr);
		
	}
	fz_always(ctx)
	{
		hk_drop_search(ctx,search);
		fz_drop_page(ctx, page);
		fz_drop_device(ctx, list_dev);
		fz_drop_display_list(ctx, list);
		fz_drop_buffer(ctx,key);
	}
	fz_catch(ctx)
	{
		jclass cls = (*env)->FindClass(env, "java/lang/OutOfMemoryError");
